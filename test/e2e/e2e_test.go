...
const namespace = "kube-headroom-system"

// serviceAccountName created for the project
const serviceAccountName = "kube-headroom-controller-manager"

// metricsServiceName is the name of the metrics service of the project
const metricsServiceName = "kube-headroom-controller-manager-metrics-service"

// metricsRoleBindingName is the name of the RBAC that will be created to allow get the metrics data
const metricsRoleBindingName = "kube-headroom-metrics-binding"
...
var _ = Describe("Manager", Ordered, func() {
	...
	Context("Manager", func() {
		...
		It("should ensure the metrics endpoint is serving metrics", func() {
			By("removing any stale ClusterRoleBinding from a prior run so create is idempotent")
			cmd := exec.Command("kubectl", "delete", "clusterrolebinding", metricsRoleBindingName,
				"--ignore-not-found")
			_, _ = utils.Run(cmd)
			...
			verifyWebhookEndpointsReady := func(g Gomega) {
				cmd := exec.Command("kubectl", "get", "endpointslices.discovery.k8s.io", "-n", namespace,
					"-l", "kubernetes.io/service-name=kube-headroom-webhook-service",
			...
			verifyMutatingWebhookReady := func(g Gomega) {
				cmd := exec.Command("kubectl", "get", "mutatingwebhookconfigurations.admissionregistration.k8s.io",
					"kube-headroom-mutating-webhook-configuration",
	...
	// Headroom's own design §10 exit criteria run inside this same Ordered
	// Describe so they reuse the already-deployed controller-manager. The specs
	// and their helpers live in headroom_test.go.
	headroomSpecs()
})
