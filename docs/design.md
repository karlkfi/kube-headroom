...
- **Memory** limits are a ratchet, not a dial — raising is free, but shrinking below current usage is best-effort (kubelet skips the resize if usage exceeds the target) and the failure mode is an OOM kill. "Ceiling tracks node slack" is incoherent for a non-reclaimable resource.
...
"Your CPU request is guaranteed. Your CPU limit is your request plus a proportional share of whatever CPU the node hasn't promised to anyone else — high on a quiet node, back to your request on a full one. If you need more sustained CPU, request more. Your limit shrinking is not an incident; it means the node got busier, and you still have everything you requested."
